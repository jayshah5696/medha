...
| 2026-04-18 | litellm (65 MB, 733 modules, 50 transitive deps) used for exactly 2 function calls. Pulled in openai SDK, tiktoken, tokenizers, cryptography, numpy | Replace with ~250 lines of httpx (llm_client.py + chat_model.py). All major LLM providers support OpenAI-compatible /v1/chat/completions. No routing library needed when the protocol is the same. |
